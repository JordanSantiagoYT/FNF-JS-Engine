...
class FreeplayState extends MusicBeatState
{
      ...
      if (!songSearchText?.hasFocus)
      {
        ...
        } else if (space)
        {
          ...
          function playSong()
          {
            ...
            final diff:String = (PlayState.SONG.specialAudioName.length > 1 ? PlayState.SONG.specialAudioName : CoolUtil.difficulties[curDifficulty])
              .toLowerCase();
            ...
            if (PlayState.SONG.needsVoices)
            {
              ...
              try
              {
                ...
                final loadedVocals:openfl.media.Sound = Paths.voices(PlayState.SONG.song, diff,
          ...
          if (instPlaying != curSelected && !player.playingMusic)
          {
            if (sys.FileSystem.exists(Paths.inst(songLowercase, CoolUtil.difficulties[curDifficulty].toLowerCase()))
              ...
              || sys.FileSystem.exists(Paths.modsJson(songLowercase + '/' + poop))) playSong();
            else
              songJsonPopup();
          }
          #else
          if (instPlaying != curSelected && !player.playingMusic)
          {
            if (OpenFlAssets.exists(Paths.inst(songLowercase + '/' + poop, CoolUtil.difficulties[curDifficulty].toLowerCase()))
              || OpenFlAssets.exists(Paths.json(songLowercase + '/' + poop))) playSong();
            else
              songJsonPopup();
          }
          #end else if (instPlaying == curSelected && player.playingMusic)
        {
  ...
  function changeSelection(change:Int = 0, playSound:Bool = true)
  {
    ...
    if (Song.hasDifficulty(songs[curSelected].songName.toLowerCase(), 'jshard')
      && ClientPrefs.JSEngineRecharts) CoolUtil.difficulties.push('jshard');
    ...
    if (songs[curSelected].songName.toLowerCase() == 'darnell')
    {
      ...
      if (ClientPrefs.JSEngineRecharts) CoolUtil.difficulties.push('jsbf');
    }
  ...
  override function beatHit()
  {
    ...
    if (curPlaying) if (grpIcons.members[instPlaying] != null
      && grpIcons.members[instPlaying].canBounce) grpIcons.members[instPlaying].bounce();
  }

...
class GameRendererSettingsSubState extends BaseOptionsMenu
{
  ...
  public function new()
  {
    ...
    final option:Option = new Option('Video Rendering Mode', // Name
    ...
    final option:Option = new Option('Video Framerate', "How much FPS would you like for your videos?", 'targetFPS', 'float', 60);
    addOption(option);
    ...
    final option:Option = new Option('Unlock Framerate', // Name
    ...
    final option:Option = new Option('Video Bitrate: ', "Use this option to set your video's bitrate!", 'renderBitrate', 'float', 5.00);
    addOption(option);
    ...
    final option:Option = new Option('Video Encoder: ', "Which video encoder would you like?\nThey all have differences like rendering speed, quality, etc.",
      'vidEncoder', 'string', 'libx264', ['libx264', 'libx264rgb', 'libx265', 'libxvid', 'libsvtav1', 'mpeg2video']);
    addOption(option);

    // I doubt this'd work on mobile (atleast for now I guess)
    #if hxnativefiledialog
    final option:Option = new Option('Output Path: ', "Where the video should be put when finished rendering. Default: 'assets/gameRenders/'", 'renderPath',
      'string', '', []);
    ...
    final option:Option = new Option('Classic Rendering Mode', // Name
      'If checked, the game will use the old Rendering Mode from 1.20.0.', 'oldFFmpegMode', 'bool', false);
    addOption(option);

    final option:Option = new Option('Lossless Screenshots', "If checked, screenshots will save as PNGs.\nOtherwise, It uses JPEG.", 'lossless', 'bool', false);
    addOption(option);

    final option:Option = new Option('JPEG Quality', "Change the JPEG quality here.\nThe recommended value is 50.", 'quality', 'int', 50);
    addOption(option);
    ...
    final option:Option = new Option('Garbage Collection Rate',
  ...
  {
    #if hxnativefiledialog
    final outPath:NFDCharStar_T = null;
    final result:NFDResult_T = NFD.PickFolder(null, cpp.RawPointer.addressOf(outPath));

...
class NotesSubState extends MusicBeatSubstate
{
  ...
  public function new()
  {
    ...
    final bg:FlxSprite = new FlxSprite(750, 160).makeGraphic(FlxG.width - 780, 540, FlxColor.BLACK);
    bg.alpha = 0.25;
    add(bg);
  ...
  final allowedTypeKeys:Map<FlxKey, String> = [
    ZERO => '0',
    ONE => '1',
    TWO => '2',
    THREE => '3',
  ...
  override function update(elapsed:Float)
  {
    ...
    if (hexTypeNum > -1)
    {
      final keyPressed:FlxKey = cast(FlxG.keys.firstJustPressed(), FlxKey);
      ...
      else if (allowedTypeKeys.exists(keyPressed))
      {
        ...
        final curColor:String = alphabetHex.text;
        final newColor:String = curColor.substring(0, hexTypeNum) + allowedTypeKeys.get(keyPressed) + curColor.substring(hexTypeNum + 1);

        final colorHex:FlxColor = FlxColor.fromString('#' + newColor);
    ...
    // Copy/Paste buttons
    final generalMoved:Bool = (FlxG.mouse.justMoved);
    final generalPressed:Bool = (FlxG.mouse.justPressed);
    if (generalMoved)
    {
      copyButton.alpha = 0.6;
      pasteButton.alpha = 0.6;
    ...
    } else if (pointerOverlaps(pasteButton))
    {
      ...
      if (generalPressed)
      {
        final formattedText = Clipboard.text.trim().toUpperCase().replace('#', '').replace('0x', '');
        final newColor:Null<FlxColor> = FlxColor.fromString('#' + formattedText);
    ...
    if (holdingOnObj != null)
    {
      ...
      } else if (generalMoved || generalPressed)
      {
        ...
        {
          final center:FlxPoint = new FlxPoint(colorWheel.x + colorWheel.width / 2, colorWheel.y + colorWheel.height / 2);
    ...
    {
      final colors:Array<FlxColor> = ClientPrefs.noteColorStyle != 'Quant-Based' ? !onPixel ? ClientPrefs.defaultArrowRGB[curSelectedNote] : ClientPrefs.defaultPixelRGB[curSelectedNote] : ClientPrefs.defaultQuantRGB[curSelectedNote];
      if (FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyJustPressed(LEFT_SHOULDER))
      {
        for (i in 0...3)
        {
          final strumRGB:RGBShaderReference = myNotes.members[curSelectedNote].rgbShader;
          var color = colors[i];
    ...
    {
      var xIndex:Float = 0;
      final lerpVal:Float = (1 - Math.exp(-48 * elapsed));
      for (i in 0...myNotes.length)
      {
        xIndex = i;
        final item = myNotes.members[i];
        if (curSelectedNote > 2 && dataArray.length > 4) xIndex -= Math.min(curSelectedNote - 2, dataArray.length - 4);

        final xPos:Float = 150 + (120 * xIndex);

        item.x += (xPos - item.x) * lerpVal;
      }
      xIndex = Math.min(curSelectedNote, dataArray.length - 2);
      if (dataArray.length > 4) if (curSelectedNote < 2) xIndex = 0;
      else
        xIndex -= 2;

      final xPos:Float = 140 - (120 * xIndex);
  ...
  function centerHexTypeLine()
  {
    ...
    if (hexTypeNum > 0)
    {
      final letter = alphabetHex.letters[hexTypeNum - 1];
      hexTypeLine.x = letter.x - letter.offset.x + letter.width;
    } else
    {
      final letter = alphabetHex.letters[0];
  ...
  {
    final text:Alphabet = new Alphabet(x, y, '', true);
    text.alignment = CENTERED;
    text.setScale(0.6);
    add(text);
    return text;
  ...
  public function spawnNotes()
  {
    ...
    for (i in 0...3)
    {
      final newNote:FlxSprite = new FlxSprite(230 + (100 * i),
    ...
    for (i in 0...dataArray.length)
    {
      ...
      final newNote:StrumNote = new StrumNote(150 + (120 * i), 200, i % 4, 0);
      ...
      if (ClientPrefs.noteColorStyle == 'Quant-Based')
      {
        final txt:AttachedText = new AttachedText(quantNames[i], 0, 0, true);
  ...
  {
    final color:FlxColor = getShaderColor();
    final wheelColor:FlxColor = specific == null ? getShaderColor() : specific;
    ...
    {
      final hueWrap:Float = wheelColor.hue * Math.PI / 180;

...
class OptionsState extends MusicBeatState
{
  ...
  override function create()
  {
    ...
    // proper fix
    if (onPlayState && PlayState.SONG.disableNoteRGB) options.remove('Note Colors');
    ...
    final yScroll:Float = Math.max(0.25 - (0.05 * (options.length - options.length)), 0.1);
  ...
  override function update(elapsed:Float)
  {
    ...
    final lerpVal:Float = CoolUtil.clamp(elapsed * 7.5, 0, 1);
    ...
    if (FlxG.keys.justPressed.ANY)
    {
      ...
      if (FlxG.keys.anyJustPressed([k]))
      {
        ...
        if (kId >= keys.length)
        {
          ...
          if (FlxG.sound.music != null) FlxTween.tween(FlxG.sound.music, {pitch: 0, volume: 0}, 2.5, {ease: FlxEase.cubeOut});

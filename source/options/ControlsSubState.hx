...
class ControlsSubState extends MusicBeatSubstate
{
  ...
  final optionShit:Array<Dynamic> = [
    ['GAMEPLAY'],
    ['Left', 'note_left'],
    ['Down', 'note_down'],
    ['Up', 'note_up'],
  ...
  public function new()
  {
    ...
    for (i in 0...optionShit.length)
    {
      ...
      final isDefaultKey:Bool = (optionShit[i][0] == defaultKey);
      if (unselectableCheck(i, true))
      {
        isCentered = true;
      }

      final optionText:Alphabet = new Alphabet(200, 300, optionShit[i][0], (!isCentered || isDefaultKey));
  ...
  override function update(elapsed:Float)
  {
    ...
    } else
    {
      ...
      if (keyPressed > -1)
      {
        final keysArray:Array<FlxKey> = ClientPrefs.keyBinds.get(optionShit[curSelected][1]);
        keysArray[curAlt ? 1 : 0] = keyPressed;

        final opposite:Int = (curAlt ? 0 : 1);
  ...
  {
    final keys:Array<Dynamic> = ClientPrefs.keyBinds.get(optionShit[num][1]);
    final text1 = new AttachedText(InputFormatter.getKeyName(keys[0]), 400, -55);
    text1.setPosition(optionText.x + 400, optionText.y - 55);
    text1.sprTracker = optionText;
    grpInputs.push(text1);
    add(text1);

    final text2 = new AttachedText(InputFormatter.getKeyName(keys[1]), 650, -55);
  ...
  function reloadKeys()
  {
    ...
    {
      final item:AttachedText = grpInputs[0];
      item.kill();
    ...
    {
      final item:AttachedText = grpInputsAlt[0];
      item.kill();

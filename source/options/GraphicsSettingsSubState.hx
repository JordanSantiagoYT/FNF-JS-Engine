...
class GraphicsSettingsSubState extends BaseOptionsMenu
{
  public function new()
  {
    ...
    final option:Option = new Option('Low Quality', // Name
    ...
    final option:Option = new Option('Anti-Aliasing', 'If unchecked, disables anti-aliasing, increases performance\nat the cost of sharper visuals.',
      'globalAntialiasing', 'bool', true);
    option.showBoyfriend = true;
    option.onChange = onChangeAntiAliasing; // Changing onChange is only needed if you want to make a special interaction after it changes the value
    addOption(option);

    final option:Option = new Option('Shaders', // Name
    ...
    final option:Option = new Option('GPU Caching', // Name
      "If checked, allows the GPU to be used for caching textures, decreasing RAM usage.\nDon't turn this on if you have a shitty Graphics Card.", // Description
      'cacheOnGPU', 'bool', false); // Don't turn this on by default
    addOption(option);

    final option:Option = new Option('Automatic Note Spawn Time', // Name
    ...
    final option:Option = new Option('Note Spawn Time', // Name
    ...
    final option:Option = new Option('Resolution: ', "What resolution do you want the game to run in?", 'resolution', 'string', '1280x720',
    ...
    final option:Option = new Option('Framerate', "Pretty self explanatory, isn't it?", 'framerate', 'int', 60);
    addOption(option);
  ...
  function onChangeAntiAliasing()
  {
    ...
    {
      final sprite:FlxSprite = Std.downcast(basic, FlxSprite);
  ...
  {
    #if desktop
    final resolutionValue = cast(ClientPrefs.resolution, String); // Assuming 'clientprefs.resolution' holds the selected resolution

    if (resolutionValue != null)
    {
      final parts = resolutionValue.split('x');

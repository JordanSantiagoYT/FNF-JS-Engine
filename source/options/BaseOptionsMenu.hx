...
class BaseOptionsMenu extends MusicBeatSubstate
{
  ...
  public function new()
  {
    ...
    for (i in 0...optionsArray.length)
    {
      ...
      } else if (optionsArray[i].type != 'link')
      {
        ...
        final valueText:AttachedText = new AttachedText('' + optionsArray[i].getValue(), optionText.width + 80);
    ...
    final buttonTop:FlxButton = new FlxButton(0, optionSearchText.y + optionSearchText.height + 5, "", function() {
  ...
  function optionsSearch(?query:String = '')
  {
    ...
    for (i in 0...originalOptionsArray.length)
    {
      ...
      {
        final optionName = originalOptionsArray[i].name.toLowerCase();
      ...
      if (nextAccept <= 0)
      {
        ...
        if (controls.RESET)
        {
          if (!FlxG.keys.pressed.SHIFT)
          {
            ...
            for (i in 0...optionsArray.length)
            {
              final leOption:Option = optionsArray[i];
              leOption.setValue(leOption.defaultValue);
              if (leOption.type != 'bool')
              {
                if (leOption.type == 'string')
  ...
  function updateTextFrom(option:Option)
  {
    ...
    final newText:String = text.replace('%v', val).replace('%d', def);

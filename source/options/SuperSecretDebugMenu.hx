...
class SuperSecretDebugMenu extends BaseOptionsMenu
{
  public function new()
  {
    ...
    final option:Option = new Option('No Ascend RNG', "If checked, makes the RNG Guns ascend part play every time.", 'noGunsRNG', 'bool', false);
    addOption(option);

    final option:Option = new Option('Crash the Engine', "Select this to crash the engine.", 'crashEngine', 'link', false);

    option.onChange = crashDaEngine;
    addOption(option);

    final option:Option = new Option('Test Update Screen', "Select this to test the engine's update screen.", 'updateThing', 'link', false);

    option.onChange = doDaUpdate;
    addOption(option);

    final option:Option = new Option('Rainbow FPS', "If checked, allows the FPS color to be rainbow.", 'rainbowFPS', 'bool', false);
    addOption(option);

    final option:Option = new Option('Force Off GC while Rendering', "If checked, the game will force GC off while rendering. Beware of memory leaks!",
